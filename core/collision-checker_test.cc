...
class CollisionCheckerFixture {
 public:
  CollisionCheckerFixture() {
    ...
    // The tests below use Actions as a proxy to verify that collisions trigger.
    ActionManager::Instance().Init();
  }

  virtual ~CollisionCheckerFixture() { ResourceManager::Instance().CleanUp(); }
 ...
 protected:
  ...
  void MoveNode(const std::string& id, const std::pair<int, int>& at) {
    ...
    // consider.
    CollisionChecker::Instance().Dirty(*node);
  }
...
SCENARIO_METHOD(CollisionCheckerFixture, "Boring collision checking",
  ...
  GIVEN("A collision checker") {
    ...
    WHEN("registered nodes do not exist") {
      CollisionChecker::Instance().RegisterCollision(
