...
def cutscene(donkey_kong, platforms, ladders):
...
def init():
    ...
    cutscene(donkey_kong, platforms, ladders)

    pytroll.events.OnEvent(
        pytroll.events.AnimationScriptDone(donkey_kong.id, 'intro'),
        lambda: pytroll.scene.ChangeScene(dk.scenes.HeightScene()))
